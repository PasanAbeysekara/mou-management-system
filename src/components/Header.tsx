                ...
                <div className="flex items-center justify-between h-16">
                    ...
                    <div className="flex items-center">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-600">University of Jayawardenapura</p>
                        </div>
                    </div>
                    ...
                    {user && (
                        <nav className="hidden md:flex space-x-8">
                            {[
                                ...
                                ...(user.role.includes('admin') ? [{ name: 'Admin Panel', path: '/admin-panel' }] : []),
                            ].map((item) => (
                                <Link
                                    key={item.path}
                                    href={item.path}
                                    className={`px-3 py-2 text-gray-700 relative group ${isActive(item.path) ? 'text-red-700' : 'hover:text-red-700'
                                        }`}
                                >
                                    {item.name}
                                    <span
                                        className={`absolute bottom-0 left-0 w-full h-0.5 bg-red-700 transform origin-left transition-transform duration-300 ${isActive(item.path) ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'
                                            }`}
                                    />
                                </Link>
                            ))}
                                    ...
                                    {showNotifications && (
                                        <div className="absolute right-0 mt-2 w-80 bg-white rounded-md shadow-lg py-1 z-50">
                                            ...
                                            <div className="px-4 py-2 border-t text-center">
                                                <Link href="/notifications" className="text-sm text-red-700 hover:text-red-800">
